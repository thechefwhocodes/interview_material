...
- Consensus check
  ...
  - Consensus matching was too hard (identifying whether models were even labeling the _same_ product attributes)
  - Fallback to no consensus but with human-review on sampled data
- **Key validation step:**
  - Silver data quality matters less in isolation
    — What matters is validating the _final fine-tuned model's_ output against the trusted human-labeled golden set.
